...
\lecturechapter{2}{Machine numbers for $\R$}
...
\begin{vbframe}{Floating point numbers in \pkg{R}}

\begin{itemize}
\item By default, \pkg{R} displays $6$ decimal places. This can be adjusted using the command \texttt{options(digits = m)}.
\item Internally, \pkg{R} calculates all floating point operations in double precision (IEEE 754, double precision, 64 bit).
...
% $$
% \frac{|\tilde x - x|}{x} \le \frac{b^{e-m}}{x}
